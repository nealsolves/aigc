...
**CLI commands (v0.9.0 beta):** `aigc workflow init`, `aigc workflow lint`, `aigc workflow doctor`, and `aigc policy init`. `aigc workflow trace` and `aigc workflow export` are reserved for PR-09.
